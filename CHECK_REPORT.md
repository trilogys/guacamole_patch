# Guacamole 输入法修复 v6 多角度审计报告

## 总体结论

v6 可以作为**受控环境部署候选版**，但在真实 Chrome/Edge → Guacamole → RDP → Windows 微软拼音链路验收通过前，不应称为已完成生产认证的正式版本。

## 对 v5 的新增修复

| 严重度 | 发现 | 影响 | v6 处理 |
| --- | --- | --- | --- |
| 高 | 长时间后台冻结后，异步 `blur()` / `focus()` 可能在 Chromium 尚未恢复原生编辑上下文时执行 | 切回后输入法继续失效，只有刷新页面才能恢复 | 记录待用户手势恢复状态，并在首次远程画面 `pointerdown` 中同步重建输入上下文 |
| 中 | 只监听 `visibilitychange` 和 `window.focus` | 页面缓存恢复或生命周期冻结/恢复可能遗漏恢复时机 | 增加 `freeze`、`resume` 和 `pageshow` 监听，同时保留原有恢复入口 |
